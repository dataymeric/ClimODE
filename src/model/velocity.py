...
def optimize_velocity(tens, tens_delta, kernel, config):
    ...
    for epoch in range(config['fitting_epoch']):
        ...
        vel_loss = criterion(tens_delta, adv) + config['regul_coeff'] * (final_x + final_y)
        vel_loss.backward()
        optimizer.step()
...
def get_batch_velocity(batch, kernel, config):
    ...
    return TensorDict({k: optimize_velocity(datas[k], deltas[k], kernel, config) for k in keys},
...
def fit_velocity(period_data, kernel, config):
    ...
    data_train = period_data.apply(lambda x: stack_past_samples(x, n=s), batch_size=[new_batch_size])

    init_data_batch_size = data_train.batch_size[0] // config['bs'] + (data_train.batch_size[0] % config['bs'] > 0)
...
def get_velocities(data, kernel, config):
    ...
    for period_name, period in data.items():
        velocities[period_name] = get_period_velocity(period_name, period, kernel, config)
    return velocities
